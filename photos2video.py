...
def convert_photos_to_video(photo_dir, output_video_path, fps=30, music_files=[],back='./back.png'):
    ...
    # 逐帧写入图片
    prev_frame = None
    for photo in tqdm.tqdm(photo_files, desc="处理图片"):
        ...
        if prev_frame is not None:
            ...
            for i in tqdm.tqdm(range(fade_frames), desc="淡入淡出", leave=False):
                alpha = i / fade_frames
                blended = cv2.addWeighted(prev_frame, 1-alpha, curr_frame, alpha, 0)
                video_writer.write(blended)
                total_duration += 1/fps
                
        # 每张图片写入3-5秒的随机帧数(不包括淡入淡出时间)
        duration = random.uniform(3, 5)  # 随机生成3-5秒的时长
        for _ in tqdm.tqdm(range(int(fps * duration)), desc="写入帧", leave=False):
